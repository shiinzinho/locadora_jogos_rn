...
function EditarJogos({ route, navigation }: any): React.JSX.Element {
    const [jogos, setJogos] = useState<Jogos>({} as Jogos);
    ...
    useEffect(() => {
        const fetchJogos = async () => {
            try {
                const response = await axios.get(`http://10.137.11.206:8000/api/jogos/${id}`);
                ...
                setDesenvolvedor(response.data.desenvolvedor);
    ...
    const atualizarJogos = async () => {
        if (validateForm()) {
            try {
                ...
                const response = await axios.put(`http://10.137.11.206:8000/api/update/game/${id}`, formData, {
                    headers: {
    ...
    return (
        <View style={styles.container}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
                ...
                <TouchableOpacity style={styles.button} onPress={atualizarJogos}>
